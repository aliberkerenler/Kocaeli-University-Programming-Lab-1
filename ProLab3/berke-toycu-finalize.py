...
def calculate_collaborators(author_input):
    author_input = author_input.strip().lower()  # Clean user input

    # Use the find_node_id function to get the matching node ID
    node_id = find_node_id(author_input)

    # If no matching node ID is found, return an error message
    if not node_id:
        return f"Hata: '{author_input}' girdisiyle eşleşen bir yazar bulunamadı."

    # Use a set to collect collaborators
    collaborators = set()

    # Find collaborators through edges
    for (source, target) in edges:
        if source == node_id or target == node_id:
            collaborators.add(source)
            collaborators.add(target)

    # Remove the author from the list of collaborators
