...
def calculate_collaborators(author_input):
    ...
    # Ağırlıklı grafiği oluşturma
    def create_weighted_graph(author_id):
        graph = {}
        for (source, target), weight in edges.items():
            if source == author_id or target == author_id:
    ...
    # Flask route for calculating shortest paths
    @app.route('/calculate_shortest_paths', methods=['POST'])
    def calculate_shortest_paths():
        author_id = request.form.get('author_id')
        if not author_id or author_id not in nodes:
            return f"Geçersiz veya bulunamayan yazar ID: {author_id}"

        weighted_graph = create_weighted_graph(author_id)
        shortest_paths = dijkstra(weighted_graph, author_id)
        ...
        for node in shortest_paths:
            ...
            current_node = node
            while current_node is not None:
                path.append(nodes[current_node]['label'])
                next_node = shortest_paths[current_node][0]
                current_node = next_node
            path = path[::-1]
            distance = shortest_paths[node][1]
            table_html += f"<tr><td>{nodes[node]['label']}</td><td>{distance}</td><td>{' ➔ '.join(path)}</td></tr>"
        table_html += "</table>"
