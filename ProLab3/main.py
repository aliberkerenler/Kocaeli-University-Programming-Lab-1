...
def shortest_path_route(start_author, end_author):
    ...
    return f"En kısa yol: {path_details}"
...
def upload_file():
            ...
            <head>
                ...
                <script>
                    ...
                    function calculateCollaborators() {{
                       const authorInput = prompt("Lütfen yazar ID'sini veya adını girin:");
                       if (authorInput) {{
                         fetch(`/calculate_collaborators/${{authorInput}}`)
                        .then(response => response.text())
                        .then(data => updateOutput(data))
                        .catch(error => updateOutput('Bir hata oluştu: ' + error));
                     }}
                    ...
                    function findLongestPath() {{
                        updateOutput('En uzun yol bulunuyor...');
                        // Buraya en uzun yol bulma işlemi ekleyin
                    }}
                </script>
            </head>
            <body>
    ...
    <h1>Graf Oluşturmak için Excel Dosyası Yükleyin</h1>
